...
Deno.serve(async (req: Request) => {
  ...
  const logisticsComplete = Boolean(logisticType && shippingMode && billableWeightKg != null)
  const confidenceStatus = logisticsComplete ? 'account_specific' : 'partial_logistics'
  const warning = logisticsComplete
    ? null
    : 'Informe logistic_type, shipping_mode e peso faturável para o fixed_fee refletir a cobrança logística do anúncio.'

  let cacheQuery = admin
    .from('live_fee_cache')
    .select('commission_pct, fixed_fee, raw_response, fetched_at, expires_at, is_exact, confidence_status, warning')
  ...
  cacheQuery = billableWeightKg != null
    ? cacheQuery.eq('billable_weight_kg', billableWeightKg)
    : cacheQuery.is('billable_weight_kg', null)
  ...
  if (cached && new Date(cached.expires_at) > new Date()) {
    return jsonResponse({
      ok: true,
      source: 'cache',
      exact: cached.is_exact,
      confidence: cached.confidence_status || confidenceStatus,
      warning: cached.warning ?? warning,
  ...
  if (billableWeightKg != null) {
    q.searchParams.set('billable_weight', String(Math.round(billableWeightKg * 1000)))
  }
  ...
  if (!feeRes.ok) {
    return jsonResponse({ ok: false, error: 'Falha na consulta de taxa no ML.', details: feeData }, 502)
  }
  ...
  if (commissionPct == null) {
    return jsonResponse({ ok: false, error: 'Resposta do ML sem percentage_fee.', details: feeData }, 502)
  }
  ...
  const { error: cacheErr } = await admin.from('live_fee_cache').upsert(
    {
      ...
      is_exact: logisticsComplete,
      confidence_status: confidenceStatus,
      warning,
    },
    {
      onConflict:
        'company_id,platform_id,category_id,listing_type,price,logistic_type,shipping_mode,billable_weight_kg',
    },
  )
