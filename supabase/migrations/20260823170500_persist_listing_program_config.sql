...
begin
  ...
  if p_product_id is null then
    ...
    values (
      v_company_id, trim(p_product->>'sku'), trim(p_product->>'name'), trim(p_product->>'category'),
      (p_product->>'cost_price')::numeric, nullif(p_product->>'weight_kg', '')::numeric, true
    )
  ...
  loop
    ...
    if v_enabled and coalesce(v_sale_price, 0) > 0 then
      insert into public.product_listings (
        product_id, platform_id, sale_price, listing_type, platform_category_id,
        logistic_type, shipping_mode, billable_weight_kg, length_cm, width_cm, height_cm,
        program_config, active
      ) values (
        v_product_id, v_platform_id, v_sale_price,
        ...
        coalesce(v_listing->'program_config', '{}'::jsonb),
        true
      )
      ...
      where product_id = v_product_id and platform_id = v_platform_id;
    end if;
  end loop;
