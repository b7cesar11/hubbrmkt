create unique index if not exists product_listings_product_platform_uidx
  on public.product_listings (product_id, platform_id);
...
begin
  ...
  loop
    ...
    if v_enabled and coalesce(v_sale_price, 0) > 0 then
      ...
      delete from public.listing_cost_components
      where product_listing_id = v_listing_id;
...
comment on function public.fn_save_product_with_listings(uuid, jsonb, jsonb) is
  'Salva produto, anúncios por marketplace, parâmetros logísticos e custos associados em uma única transação, sempre limitado à empresa do usuário autenticado.';
