alter table public.platform_fee_rules
  drop constraint if exists no_overlapping_fee_rules;
...
alter table public.platform_fee_rules
  ...
  exclude using gist (
    ...
    coalesce(listing_type, '') with =,
    coalesce(reputation_level, '') with =,
...
begin
  ...
  if v_old.valid_to < current_date then
...
revoke all on function public.fn_version_fee_rule(uuid,numeric,numeric,text,text,text,jsonb) from public, anon;
